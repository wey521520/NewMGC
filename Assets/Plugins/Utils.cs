...
public static class Utils
{
	...
	/// <param name="data">Data.</param>
	/// <param name="key">Key.</param>
	/// <param name="default_val">Default_val.</param>
	/// <typeparam name="T">The 1st type parameter.</typeparam>
	public static T getKeyValue<T> (JsonData data, string key, T default_val)
	{
		object t = default_val;
		try {
			JsonData v = data [key];
			if (typeof(T) == typeof(int)) {
				...
				throw new Exception (string.Format ("unknow T type: {0}", typeof(T)));
			}
			
		} catch (Exception e) {
			Utils.print (LogLevel.WARNING, "failed get key:{0} from {1} by {2}", key, data.ToJson (), e.ToString ());
		}
		return (T)t;
	}
...
public class TraceLog
{
	...
	public void time (string tag)
	{
		Utils.print (LogLevel.DEBUG, "time cost: {0}ms [{1}]", (System.DateTime.Now.Ticks - _start) / 10000, tag); 
	}
